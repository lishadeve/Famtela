...
namespace Famtela.Application.Features.Growers.Commands.Import
{
    public partial class ImportGrowersCommand : IRequest<Result<int>>
    {
        public UploadRequest UploadRequest { get; set; }
    }

    internal class ImportGrowersCommandHandler : IRequestHandler<ImportGrowersCommand, Result<int>>
    {
        private readonly IUnitOfWork<int> _unitOfWork;
        private readonly IExcelService _excelService;
        private readonly IMapper _mapper;
        private readonly IValidator<AddEditGrowerCommand> _addGrowerValidator;
        private readonly IStringLocalizer<ImportGrowersCommandHandler> _localizer;

        public ImportGrowersCommandHandler(
            IUnitOfWork<int> unitOfWork,
            IExcelService excelService,
            IMapper mapper,
            IValidator<AddEditGrowerCommand> addGrowerValidator,
            IStringLocalizer<ImportGrowersCommandHandler> localizer)
        {
            _unitOfWork = unitOfWork;
            _excelService = excelService;
            _mapper = mapper;
            _addGrowerValidator = addGrowerValidator;
            _localizer = localizer;
        }

        public async Task<Result<int>> Handle(ImportGrowersCommand request, CancellationToken cancellationToken)
        {
            var stream = new MemoryStream(request.UploadRequest.Data);
            var result = (await _excelService.ImportAsync(stream, mappers: new Dictionary<string, Func<DataRow, Grower, object>>
            {
            ...
            if (result.Succeeded)
            {
                var importedGrowers = result.Data;
                var errors = new List<string>();
                var errorsOccurred = false;
                foreach (var grower in importedGrowers)
                {
                    var validationResult = await _addGrowerValidator.ValidateAsync(_mapper.Map<AddEditGrowerCommand>(grower), cancellationToken);
                    ...
                    {
                        errorsOccurred = true;
                        errors.AddRange(validationResult.Errors.Select(e => $"{(!string.IsNullOrWhiteSpace(grower.TypeofFeed) ? $"{grower.TypeofFeed} - " : string.Empty)}{e.ErrorMessage}"));
                    }
                }
